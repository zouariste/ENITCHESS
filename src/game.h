...
void Game::startgame() {
    ...
    while (window.isOpen()) {
        ...
        window.draw(f[n]);
        window.draw(f[33]);
        if ((this -> nbtour) % 2) window.draw(f[34]);
        else window.draw(f[32]);
        for (int i = 0; i < 32; i++) f[i].move(-offset);
        window.display();
        if ((s == 1) || ((mode != 1) && (nbtour % 2 == 0))) {
            ...
            if (test) {
                ...
                //std::cout << "move"<<ini.x << ' ' << ini.y << ' ' << dest.x << ' ' << dest.y << endl;
...
void Game::reprendreGame() {
    ...
    if (fichier.is_open()) {
        ...
        fichier >> this -> nbtour;
        fichier >> difficulty;
        if (difficulty == 0) this -> white = new Human;
        else this -> white = new Ai(difficulty);
        fichier >> difficulty;
        if (difficulty == 0) this -> black = new Human;
        else this -> black = new Ai(difficulty);
        difsim = difficulty;
        this -> black -> color = 2;
        this -> white -> color = 1;
