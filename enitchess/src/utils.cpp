...
template<class T>
void LinkedList<T>::affichechaine()
{
...
void LinkedList<T>::addlist(const T data)
{
    ...
    {
        cCell<T> *cour;cour=new cCell<T>(data);cour->succ=head;
        head=cour;
...
template<class T>
int LinkedList<T>::cherchechaine(const T data) const
{
    if (this->head != NULL) if (this->head->elt==data) return 1;
